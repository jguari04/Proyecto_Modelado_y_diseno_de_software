...
def build_section(parent, title):
    """Crea un marco con título en estilo ttkbootstrap."""
    import ttkbootstrap as tb
    frm_wrap = tb.Labelframe(parent, text=title)
    frm_body = tb.Frame(frm_wrap, padding=5)
    frm_body.pack(fill="both", expand=True)
    return frm_wrap, frm_body
...
class MPApp(tb.Window):
    ...
    def _build_header(self):
        ...
        # Derecha: selector de ámbito + botones
        right = ttk.Frame(hdr)
        right.pack(side=tk.RIGHT)

        row = ttk.Frame(right)
        row.pack(anchor="e", pady=(0, 6))
        ttk.Label(row, text="Ámbito:").pack(side=tk.LEFT, padx=(0, 6))
        self.cmb_scope = ttk.Combobox(row, state="readonly", values=["Mes", "Año", "General"], width=10)
        # si tu app ya no usa "General", podés dejar ["Mes","Año"]
        # y setear el índice 0 (Mes)
        if "General" in self.cmb_scope["values"]:
            self.cmb_scope.set("Mes")
        else:
            self.cmb_scope.set("Mes")
        self.cmb_scope.pack(side=tk.LEFT)
    ...
    def _build_body(self):
        ...
        self.left_list = ttk.Frame(left_body)
        ...
        # Atajo de teclado: Supr para eliminar fila seleccionada
